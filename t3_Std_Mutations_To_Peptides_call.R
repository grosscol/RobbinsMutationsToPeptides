################################################################################
# Title:  Standard Input Mutation Data --> Peptides                            #
# Date:   2012-07-09                                                           #
...
#Check that a UCSC database is loaded. else load default hg19
validdbnames <- c("package:BSgenome.Hsapiens.UCSC.hg18", 
                  "package:BSgenome.Hsapiens.UCSC.hg19")

dbsloaded <- unlist(sapply(validdbnames, FUN=function(v) grep(v,search(),value=TRUE)) )

if( length(dbsloaded) != 1 ){
  cat("Loaded valid DBs not equal to 1.\n")
  cat("Loaded DBs:",length(dbsloaded),"\n",paste(dbsloaded,sep='\n'))
  #detach loaded packages
  sapply(dbsloaded, FUN=detach, character.only=TRUE)
  #attach default
...
#################################################
###  Input data should be part of        #######
### parent environment, or die          #######
##############################################
if( !("dfc" %in% ls()) ){ stop('Input data.frame not found.') }
